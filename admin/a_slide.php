...
if (isset($_POST['envio'])) {
    ...
    if ($errorMessage === '') {
        if ($id_slide > 0) {
            $stmt = mysqli_prepare($con, 'UPDATE tbl_slider SET imagen=?, link_url=?, orden=?, activo=? WHERE id_slide=?');
            $linkParam = $link_url !== '' ? $link_url : null;
            mysqli_stmt_bind_param($stmt, 'ssiii', $imagen, $linkParam, $orden, $activo, $id_slide);
            mysqli_stmt_execute($stmt);
            header('Location: slider.php?modificado=1');
        } else {
            $stmt = mysqli_prepare($con, 'INSERT INTO tbl_slider (imagen, link_url, orden, activo) VALUES (?, ?, ?, ?)');
            $linkParam = $link_url !== '' ? $link_url : null;
            mysqli_stmt_bind_param($stmt, 'ssii', $imagen, $linkParam, $orden, $activo);
            mysqli_stmt_execute($stmt);
...
<div class="admin-card">
    <div class="admin-card-body">
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label class="form-label" for="imagen"><?= $id_slide > 0 ? 'Reemplazar imagen' : 'Imagen' ?></label>
